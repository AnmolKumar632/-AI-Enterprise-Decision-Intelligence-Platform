...
| **QA-BUG-007** | Reports | Slide Builder | Compile executive PowerPoint deck. | Build slide deck without errors. | Output compilation fails with AttributeError on text_frame attribute. | **FAIL** | High | High | Typo in recommendations slide builder: called `.text_frame` on a variable (`tf`) that was already a `TextFrame` object. | Replace `tf.text_frame` with `tf` on line 232 of `apps/report_generator/generator.py`. | **CLOSED** |
